...
pub fn find_pid_by_name(name: &str) -> Option<u32> {
    ...
    let snapshot = ManagedHandle::new(raw_snapshot)?;
...
pub fn open_process(pid: u32, access_right: Option<PROCESS_ACCESS_RIGHTS>) -> Option<ManagedHandle> {
    let ar = access_right.unwrap_or(PROCESS_ALL_ACCESS);
    let h: HANDLE = unsafe { OpenProcess(ar, FALSE, pid) };
    let oh = ManagedHandle::new(h)?;
